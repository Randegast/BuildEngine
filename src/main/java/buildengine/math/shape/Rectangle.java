...
public class Rectangle extends Polygon {
    ...
    public Rectangle(Transform transform) {
        this(transform.position.x - transform.width / 2, transform.position.y - transform.height / 2, transform.width, transform.height, transform.rotation);
    }
    ...
    public Vector2f[] getAxes() {
        ...
        axes[0] = vertices[1].sub(vertices[0], new Vector2f()).perpendicular().normalize();
        axes[1] = vertices[2].sub(vertices[1], new Vector2f()).perpendicular().normalize();
        return axes;
    }
